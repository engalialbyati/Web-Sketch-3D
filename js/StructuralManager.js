...
(function (root) {
  ...
  class StructuralManager {
    ...
    columnBearingTop(p, pool = null) {
      // v0.7 RESTORED (owner's rule): a column runs THROUGH the beam
      // zone to its own top constraint - the intersection cube is filled
      // by the column, and the beams stop at the column faces (the
      // framing trim in buildBeam). The RC beam/column/slab cages are
      // interconnected and need this geometry.
    ...
    buildBeam(G, model, p, pending) {
      ...
      // RESTORED FRAMING TRIM (owner's rule): the beam ENDS AT THE COLUMN
      // FACE - the sweep is trimmed back by the column's plan half-extent
      // along the run direction (the support function of its box). No
      // column: extend webWidth/2 to weld into an empty corner. Either way
      // the PARAMS keep the analytical centerline-to-centerline baseline.
